...
# 1. Update index.html and employee.html declineMeetingCall function
with open('index.html', 'r', encoding='utf-8') as f:
    idx_html = f.read()

old_decline = """        function declineMeetingCall() {
...
new_decline = """        function declineMeetingCall() {
            var modal = document.getElementById('incomingMeetingModal');
            if(modal) modal.style.display = 'none';
            stopMeetingRinging();
            window._declinedMeeting = true;
            if(window._incomingCallData && window._incomingCallData.meetingId && window.db) {
                db.collection('meetings').doc(window._incomingCallData.meetingId).update({
                    status: 'rejected'
                }).catch(function(e){console.error(e)});
            }
        }"""

idx_html = idx_html.replace(old_decline, new_decline)

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(idx_html)
print("index.html declineMeetingCall updated with Firestore rejection status!")


with open('employee.html', 'r', encoding='utf-8') as f:
    emp_html = f.read()

emp_html = emp_html.replace(old_decline, new_decline)

with open('employee.html', 'w', encoding='utf-8') as f:
    f.write(emp_html)
print("employee.html declineMeetingCall updated with Firestore rejection status!")


# 2. Update app.js with Google Meet architecture
with open('app.js', 'r', encoding='utf-8') as f:
    js = f.read()

google_meet_code = """var _jitsiWindowRef = null;
var _currentRoomUrl = '';
        ...
        if(!isGroup) {
            ...
            window._callStatusUnsubscribe = db.collection('meetings').doc(_currentMeetingId).onSnapshot(function(doc) {
                ...
                if(d.status === 'rejected') {
                    alert("❌ تم رفض المكالمة من قبل الموظف.");
                    endCall();
    ...
    db.collection('meetings').where('status', 'in', ['calling', 'active']).onSnapshot(function(snap) {
        ...
        snap.forEach(function(doc) {
            ...
            if (data.status === 'calling' && createdAt && (Date.now() - createdAt.getTime() > 45000)) {
                ...
                if (data.targetUid === TG_USER.uid && typeof stopMeetingRinging === 'function') {
                    var modal = document.getElementById('incomingMeetingModal');
                    if(modal) modal.style.display = 'none';
                    stopMeetingRinging();
                }
...
if start_pos != -1 and end_pos != -1:
    js = js[:start_pos] + google_meet_code + "\n\n" + js[end_pos:]
    print("app.js Google Meet architecture applied successfully!")
else:
    print("Could not match bounds in app.js!")

with open('app.js', 'w', encoding='utf-8') as f:
    f.write(js)
print("Complete Google Meet Architecture Script!")
