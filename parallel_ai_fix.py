...
new_func = """function callGemini(apiKey, prompt, btn, resultBox, btnOriginalText, isAdmin) {
    ...
    .then(function(data) {
        ...
        if (selectedModels.length === 0) throw new Error("لم يتم العثور على أي نموذج.");

        resultBox.innerHTML = '<div style="text-align:center; color:var(--tx2);">جاري إنشاء الرد بأقصى سرعة...</div>';

        function fetchModel(modelName) {
            return fetch('https://generativelanguage.googleapis.com/v1beta/' + modelName + ':generateContent?key=' + apiKey, {
                method: 'POST',
        ...
        function handleError(err) {
            if (resolved) return;
            errors++;
            if (errors === selectedModels.length) {
                btn.disabled = false;
                btn.innerHTML = btnOriginalText;
                resultBox.innerHTML = '<div style="color:red;">❌ عذراً، جميع النماذج المتاحة بحسابك لم تنجح في الرد.</div>';
            }
        }

        for(var j=0; j<selectedModels.length; j++) {
            fetchModel(selectedModels[j]).then(handleSuccess).catch(handleError);
        }
    })
    .catch(function(err) {
        btn.disabled = false;
...
if start_idx != -1 and end_idx != -1:
    ...
    with open('app.js', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Successfully replaced callGemini with parallel execution approach")
else:
